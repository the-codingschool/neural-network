...
I altered an existing pytorch neural network based on the mscii database in Google Colab recommended to me by my mentor. I then loaded the data into the neural network and filtered each image so they were all 100 by 100 pixels.

**Research Question**

Which set of neural network perameters yield the highest accuracy?

**Hypotheses**

Null: The peramaters of the neural network do not impact its accuracy whatsoever.

Alternate: Altering the perameters of the neural network causes a profound difference in the results of the network.
...
**Conclusion**
From my research, it is evident that a neural network is capable of identifying categories of tissue with some degree of accuracy. In the future, it may be possible to reach an accuracy and consistency that is high enough for commercial medical use, greatly decreasing both time and monetary constraints on receiving treatment for cancer and other diseases. If I had more time on this project, I would experiment more with the peramaters of the neural network. Though I found one local minima, it is unlikely to be the global minima, so the neural network is probably capable of a much higher accuracy than 52%. Additionally, I would like to have experimented with
